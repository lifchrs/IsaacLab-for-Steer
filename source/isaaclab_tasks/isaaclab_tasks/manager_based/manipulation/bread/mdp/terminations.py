...
def task_done_insert(
    ...
    bread: RigidObject = env.scene[bread_cfg.name]
    toaster: RigidObject = env.scene[toaster_cfg.name]

    pos_diff = bread.data.root_pos_w - toaster.data.root_pos_w
    height_dist = pos_diff[:, 2]
    xy_dist = torch.linalg.vector_norm(pos_diff[:, :2], dim=1)
    height_diff = abs(height_dist - height_threshold)

    done = torch.logical_and(xy_dist < xy_threshold, height_diff < height_threshold)

    bread_rot = bread.data.root_quat_w

    angle_with_z_axis = torch.arccos(
        1 - 2 * (bread_rot[:, 1] ** 2 + bread_rot[:, 2] ** 2)
    )
    # print(f"angle_with_z_axis: {angle_with_z_axis}")

    # bread should be placed upright
