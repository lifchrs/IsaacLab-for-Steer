...
def randomize_object_pose(
    ...
    env_ids: torch.Tensor,
    pose_range: dict[str, tuple[float, float]],
    bread_cfg: SceneEntityCfg = SceneEntityCfg("bread"),
    toaster_cfg: SceneEntityCfg = SceneEntityCfg("toaster"),
    ...
    for cur_env in env_ids.tolist():
        ...
        # Randomize pose for each object, the egg and the bread are placed together
        bread = env.scene[bread_cfg.name]
        toaster = env.scene[toaster_cfg.name]
        egg = env.scene[egg_cfg.name]

        # Write pose to simulation for the bread and the egg
        ...
        bread.write_root_pose_to_sim(
            torch.cat([positions, orientations], dim=-1),
            env_ids=torch.tensor([cur_env], device=env.device),
        )
        bread.write_root_velocity_to_sim(
            torch.zeros(1, 6, device=env.device),
            env_ids=torch.tensor([cur_env], device=env.device),
        )

        egg.write_root_pose_to_sim(
            torch.cat([positions, orientations], dim=-1),
            env_ids=torch.tensor([cur_env], device=env.device),
        )
        egg.write_root_velocity_to_sim(
            torch.zeros(1, 6, device=env.device),
            env_ids=torch.tensor([cur_env], device=env.device),
        )

        # Write pose to simulation for the toaster
        pose_tensor = torch.tensor([pose_list[1]], device=env.device)
        positions = pose_tensor[:, 0:3] + env.scene.env_origins[cur_env, 0:3]
        orientations = math_utils.quat_from_euler_xyz(
            pose_tensor[:, 3], pose_tensor[:, 4], pose_tensor[:, 5]
        )

        toaster.write_root_pose_to_sim(
            torch.cat([positions, orientations], dim=-1),
            env_ids=torch.tensor([cur_env], device=env.device),
        )
        toaster.write_root_velocity_to_sim(
            torch.zeros(1, 6, device=env.device),
            env_ids=torch.tensor([cur_env], device=env.device),
        )
...
def change_egg_body_com(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | None,
    com_range: dict[str, tuple[float, float]],
    asset_cfg: SceneEntityCfg = SceneEntityCfg("egg"),
):
    """Randomize the center of mass (CoM) of egg by adding a random value sampled from the given ranges.
    ...
    coms[env_ids[:, None], :3] += rand_samples
